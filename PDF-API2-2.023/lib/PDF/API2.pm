...
sub calc_length_height_of_sentence {
	my $self = shift;
	my $page = shift;
	my $sentence = shift;
	my $cell_font = shift;
	my $cell_font_size = shift;
	my $max_word_len = 20;
	
	my $txt     = $page->text;
	
	
    my $column_width = 0; #holds the width of each column
    ...
    if( $max_word_len > 0 )
    {
    	$sentence =~ s#(\S{$max_word_len})(?=\S)#$1 #g;  
    }
            
    # Init cell size limits  
    my $space_w                      = $txt->advancewidth( "\x20" );
