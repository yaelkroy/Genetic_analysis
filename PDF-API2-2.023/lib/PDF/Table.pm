...
sub table
{
    ...
    my $fnt_name    = $arg{'font'            } || $pdf->ttfont('Arial.ttf',-encode => 'utf8');
    my $fnt_size    = $arg{'font_size'       } || 8;
    ...
    while(scalar(@{$data}))
    {
        ...
        while(scalar(@{$data}) and $cur_y-$row_h > $bot_marg)
        {
            ...
            for(my $column_idx = 0 ; $column_idx < scalar(@$record) ; $column_idx++)
            {
                ...
                $cur_x += $calc_column_widths->[$column_idx];
                $no_hline_width +=  $calc_column_widths->[$column_idx] if exists  $cell_props->[$row_index][$column_idx]->{'NO_HLINE'};
                #if (exists  $cell_props->[$row_index][$column_idx]->{'NO_HLINE'}) {
                #	print "$row_index\t$column_idx\n";
                #}
            }#End of for(my $column_idx....
            ...
            {
                $gfx->move(  $xbase + $no_hline_width , $cur_y );
                $gfx->hline( $xbase + $width );
            }
